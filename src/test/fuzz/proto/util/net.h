...
#include <test/fuzz/proto/util/net.pb.h>
 ...
 * @param[in,out] m Take data for the address from this, if `rand` is `nullptr`.
...
{
public:
    explicit AddrManDeterministic(const NetGroupManager& netgroupman, const proto_fuzz_util_net::AddrManDeterministic& m, int32_t check_ratio)
        : AddrMan(netgroupman, /*deterministic=*/true, check_ratio)
    {
        WITH_LOCK(m_impl->cs, m_impl->insecure_rand.Reseed(ConsumeUInt256(m.seed())));
    }
...
{
    const proto_fuzz_util_net::FuzzSockInternal& m_data_provider;
    const proto_fuzz_util_net::FuzzSockAccept* m_accept_provider;

    mutable uint32_t nextSendElement = 0;
    mutable uint32_t nextRecvElement = 0;
    mutable uint32_t nextRecvMessage = 0;
    mutable uint32_t nextAcceptElement = 0;
    mutable uint32_t nextSetSockOptElement = 0;
    mutable uint32_t nextGetSockOptElement = 0;
    mutable uint32_t nextWaitElement = 0;

    mutable uint32_t m_remain_before_disconnected;
    ...
    bool WaitInternal(std::optional<std::chrono::milliseconds> timeout, Event requested, Event* occurred) const;

public:
    explicit FuzzedSock(const proto_fuzz_util_net::FuzzSockInternal& data_provider,
                        const proto_fuzz_util_net::FuzzSockAccept* accept_provider = nullptr);

    explicit FuzzedSock(const proto_fuzz_util_net::FuzzSockAccept& data_provider)
            : FuzzedSock(data_provider.socket(), &data_provider) {}
    ...
    bool Wait(std::chrono::milliseconds timeout, Event requested, Event* occurred = nullptr) const override {
        return WaitInternal(timeout, requested, occurred);
    }
...
[[nodiscard]] inline NetGroupManager ConsumeNetGroupManager(const proto_fuzz_util_net::NetGroupManager& m) noexcept
{
    std::vector<bool> asmap = ConsumeBitsVector(m.value());
...
inline CService ConsumeService(const proto_fuzz_util_net::Service& m) noexcept
{
    return {ConsumeNetAddr(m.netaddr()), ConsumeIntegralInRange<uint16_t>(m.port())};
}

CAddress ConsumeAddress(const proto_fuzz_util_net::Address& m) noexcept;

template <bool ReturnUniquePtr = false>
auto ConsumeNode(const proto_fuzz_util_net::Node& m, const std::optional<NodeId>& node_id_in = std::nullopt) noexcept
{
    const NodeId node_id = node_id_in.value_or(ConsumeIntegralInRange<NodeId>(m.nodeid(), 0, std::numeric_limits<NodeId>::max()));
    const auto sock = std::make_shared<FuzzedSock>(m.fuzz_sock());
    const CAddress address = ConsumeAddress(m.address());
    const uint64_t keyed_net_group = m.keyed_net_group();
    const uint64_t local_host_nonce = m.local_host_nonce();
    const CAddress addr_bind = ConsumeAddress(m.addr_bind());
    const std::string addr_name = m.addr_name();
    const ConnectionType conn_type = ConsumeEnum(m.conn_type(), ALL_CONNECTION_TYPES);
    const bool inbound_onion{conn_type == ConnectionType::INBOUND ? m.inbound_onion() : false};
    NetPermissionFlags permission_flags = ConsumeWeakEnum(m.permission_flags(), ALL_NET_PERMISSION_FLAGS);
...
inline std::unique_ptr<CNode> ConsumeNodeAsUniquePtr(const proto_fuzz_util_net::Node& m,
                                                     const std::optional<NodeId>& node_id_in = std::nullopt) {
    return ConsumeNode<true>(m, node_id_in);
}
