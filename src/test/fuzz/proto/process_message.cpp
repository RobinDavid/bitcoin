...
#include <sync.h>
#include <test/fuzz/proto/process_message.pb.h>
...
FUZZ_PROTO_TARGET(process_message, const process_message_fuzz::Target& el, .init = initialize_process_message) {
    ...
    const std::string random_message_type = ConsumeString(el.random_message_type(), CMessageHeader::MESSAGE_TYPE_SIZE).c_str();
    if (!LIMIT_TO_MESSAGE_TYPE.empty() && random_message_type != LIMIT_TO_MESSAGE_TYPE) {
        return;
    }
    CNode& p2p_node = *ConsumeNodeAsUniquePtr(el.node()).release();

    connman.AddTestNode(p2p_node);
    FillNode(el.nodeinfo(), connman, p2p_node);

    const auto mock_time = ConsumeTime(el.time());
    SetMockTime(mock_time);

    CSerializedNetMsg net_msg;
    net_msg.m_type = random_message_type;
    net_msg.data = ConsumeByteVector(el.content(), MAX_PROTOCOL_MESSAGE_LENGTH);
