...
#include <test/fuzz/proto/txdownloadman.pb.h>
...
static std::vector<COutPoint> PickCoins(const txdownloadman_fuzz::COutPoint& el) {
    std::vector<COutPoint> ret;
    ret.push_back(ConsumeEnum(el.firstcoin(), COINS));
    for (int i = 0; i < std::min(el.nextcoins_size(), 10); i++) {
        ret.push_back(ConsumeEnum(el.nextcoins(i), COINS));
    }
    return ret;
}
...
FUZZ_PROTO_TARGET(txdownloadman, const txdownloadman_fuzz::Target_txdownloadman& el, .init = initialize) {
    ...
    const auto max_orphan_count = ConsumeIntegralInRange<unsigned int>(el.max_orphan_count(), 0, 300);
    ...
    for (unsigned i = 0; i < std::min<unsigned>(el.elements_size(), 10000); i++) {
        const auto& currentElement = el.elements(i);
        NodeId rand_peer = ConsumeIntegralInRange<int64_t>(currentElement.rand_peer(), 0, NUM_PEERS - 1);

        // Transaction can be one of the premade ones or a randomly generated one
        auto rand_tx = currentElement.has_newrandtx() ?
            MakeTransactionSpending(PickCoins(currentElement.newrandtx().coin()),
                                    /*num_outputs=*/ConsumeIntegralInRange<unsigned>(currentElement.newrandtx().num_outputs(), 1, 500),
                                    /*add_witness=*/currentElement.newrandtx().add_witness()) :
            TRANSACTIONS.at(ConsumeIntegralInRange<unsigned>(currentElement.existingrandtx(), 0, TRANSACTIONS.size() - 1));

        switch (currentElement.action_type_case()) {
            default:
            case txdownloadman_fuzz::Target_txdownloadman_LoopElement::kConnectedPeer: {
                node::TxDownloadConnectionInfo info{
                    .m_preferred = currentElement.connectedpeer().preferred(),
                    .m_relay_permissions = currentElement.connectedpeer().relay_permissions(),
                    .m_wtxid_relay = currentElement.connectedpeer().wtxid_relay()
                };
                txdownloadman.ConnectedPeer(rand_peer, info);
                break;
            }
            case txdownloadman_fuzz::Target_txdownloadman_LoopElement::kDisconnectedPeer: {
                txdownloadman.DisconnectedPeer(rand_peer);
                txdownloadman.CheckIsEmpty(rand_peer);
                break;
            }
            case txdownloadman_fuzz::Target_txdownloadman_LoopElement::kActiveTipChange: {
                txdownloadman.ActiveTipChange();
                break;
            }
            case txdownloadman_fuzz::Target_txdownloadman_LoopElement::kBlockConnected: {
                CBlock block;
                block.vtx.push_back(rand_tx);
                txdownloadman.BlockConnected(std::make_shared<CBlock>(block));
                break;
            }
            case txdownloadman_fuzz::Target_txdownloadman_LoopElement::kBlockDisconnected: {
                txdownloadman.BlockDisconnected();
                break;
            }
            case txdownloadman_fuzz::Target_txdownloadman_LoopElement::kMempoolAcceptedTx: {
                txdownloadman.MempoolAcceptedTx(rand_tx);
                break;
            }
            case txdownloadman_fuzz::Target_txdownloadman_LoopElement::kMempoolRejectedTx: {
                TxValidationState state;
                state.Invalid(PickValueInArray(currentElement.mempoolrejectedtx().result(), TESTED_TX_RESULTS), "");
                bool first_time_failure{currentElement.mempoolrejectedtx().first_time_failure()};
            ...
            case txdownloadman_fuzz::Target_txdownloadman_LoopElement::kAddTxAnnouncement: {
                GenTxid gtxid = currentElement.addtxannouncement() ?
                                GenTxid::Txid(rand_tx->GetHash()) :
                                GenTxid::Wtxid(rand_tx->GetWitnessHash());
                txdownloadman.AddTxAnnouncement(rand_peer, gtxid, time);
                break;
            }
            case txdownloadman_fuzz::Target_txdownloadman_LoopElement::kGetRequestsToSend: {
                txdownloadman.GetRequestsToSend(rand_peer, time);
                break;
            }
            case txdownloadman_fuzz::Target_txdownloadman_LoopElement::kReceivedTx: {
            ...
            case txdownloadman_fuzz::Target_txdownloadman_LoopElement::kReceivedNotFound: {
                txdownloadman.ReceivedNotFound(rand_peer, {rand_tx->GetWitnessHash()});
                break;
            }
            case txdownloadman_fuzz::Target_txdownloadman_LoopElement::kGetTxToReconsider: {
        ...
        auto time_skip = PickValueInArray(currentElement.time_skip(), TIME_SKIPS);
        if (currentElement.minus_time_skip()) time_skip *= -1;
        time += time_skip;
    }
...
FUZZ_PROTO_TARGET(txdownloadman_impl, const txdownloadman_fuzz::Target_txdownloadman_impl& el, .init = initialize) {
    ...
    for (unsigned i = 0; i < std::min<unsigned>(el.elements_size(), 10000); i++) {
        ...
        switch (currentElement.action_type_case()) {
            default:
            case txdownloadman_fuzz::Target_txdownloadman_impl_LoopElement::kConnectedPeer: {
                node::TxDownloadConnectionInfo info{
                    .m_preferred = currentElement.connectedpeer().preferred(),
            ...
            case txdownloadman_fuzz::Target_txdownloadman_impl_LoopElement::kDisconnectedPeer: {
                txdownload_impl.DisconnectedPeer(rand_peer);
                txdownload_impl.CheckIsEmpty(rand_peer);
                break;
            }
            case txdownloadman_fuzz::Target_txdownloadman_impl_LoopElement::kActiveTipChange: {
            ...
            case txdownloadman_fuzz::Target_txdownloadman_impl_LoopElement::kBlockConnected: {
                CBlock block;
                block.vtx.push_back(rand_tx);
            ...
            case txdownloadman_fuzz::Target_txdownloadman_impl_LoopElement::kBlockDisconnected: {
            ...
            case txdownloadman_fuzz::Target_txdownloadman_impl_LoopElement::kMempoolAcceptedTx: {
                txdownload_impl.MempoolAcceptedTx(rand_tx);
                break;
            }
            case txdownloadman_fuzz::Target_txdownloadman_impl_LoopElement::kMempoolRejectedTx: {
                TxValidationState state;
                state.Invalid(PickValueInArray(currentElement.mempoolrejectedtx().result(), TESTED_TX_RESULTS), "");
                bool first_time_failure{currentElement.mempoolrejectedtx().first_time_failure()};
            ...
            case txdownloadman_fuzz::Target_txdownloadman_impl_LoopElement::kAddTxAnnouncement: {
                GenTxid gtxid = currentElement.addtxannouncement() ?
                                GenTxid::Txid(rand_tx->GetHash()) :
                                GenTxid::Wtxid(rand_tx->GetWitnessHash());
            ...
            case txdownloadman_fuzz::Target_txdownloadman_impl_LoopElement::kGetRequestsToSend: {
            ...
            case txdownloadman_fuzz::Target_txdownloadman_impl_LoopElement::kReceivedTx: {
            ...
            case txdownloadman_fuzz::Target_txdownloadman_impl_LoopElement::kReceivedNotFound: {
                txdownload_impl.ReceivedNotFound(rand_peer, {rand_tx->GetWitnessHash()});
                break;
            }
            case txdownloadman_fuzz::Target_txdownloadman_impl_LoopElement::kGetTxToReconsider: {
                ...
                if (ptx) {
                    ...
                    txdownload_impl.MempoolRejectedTx(ptx, state_missing_inputs, rand_peer, currentElement.gettxtoreconsider());
                }
                break;
            }
        }
