...
#include <test/fuzz/proto/coinscache_sim.pb.h>
...
FUZZ_PROTO_TARGET(coinscache_sim, const coinscache_sim_fuzz::Target& el) {
    ...
    for (unsigned i = 0; i < std::min<unsigned>(el.actions_size(), 10000); i++) {
        ...
        switch (currentAction.action_type_case()) {
            default:
            case coinscache_sim_fuzz::Target_Action::kGetCoin: { // GetCoin
                uint32_t outpointidx = ConsumeIntegralInRange<uint32_t>(currentAction.getcoin(), 0, NUM_OUTPOINTS - 1);
            ...
            case coinscache_sim_fuzz::Target_Action::kHaveCoin: { // HaveCoin
                uint32_t outpointidx = ConsumeIntegralInRange<uint32_t>(currentAction.havecoin(), 0, NUM_OUTPOINTS - 1);
                // Look up in simulation data.
                auto sim = lookup(outpointidx);
                // Look up in real caches.
            ...
            case coinscache_sim_fuzz::Target_Action::kHaveCoinInCache: { // HaveCoinInCache
                uint32_t outpointidx = ConsumeIntegralInRange<uint32_t>(currentAction.havecoinincache(), 0, NUM_OUTPOINTS - 1);
            ...
            case coinscache_sim_fuzz::Target_Action::kAccessCoin: { // AccessCoin
                uint32_t outpointidx = ConsumeIntegralInRange<uint32_t>(currentAction.accesscoin(), 0, NUM_OUTPOINTS - 1);
                // Look up in simulation data.
                auto sim = lookup(outpointidx);
                // Look up in real caches.
            ...
            case coinscache_sim_fuzz::Target_Action::kAddCoin: { // AddCoin (only possible_overwrite if necessary)
                uint32_t outpointidx = ConsumeIntegralInRange<uint32_t>(currentAction.addcoin().outpointidx(), 0, NUM_OUTPOINTS - 1);
                uint32_t coinidx = ConsumeIntegralInRange<uint32_t>(currentAction.addcoin().coinidx(), 0, NUM_COINS - 1);
            ...
            case coinscache_sim_fuzz::Target_Action::kAddCoinAlways: { // AddCoin (always possible_overwrite)
                uint32_t outpointidx = ConsumeIntegralInRange<uint32_t>(currentAction.addcoinalways().outpointidx(), 0, NUM_OUTPOINTS - 1);
                uint32_t coinidx = ConsumeIntegralInRange<uint32_t>(currentAction.addcoinalways().coinidx(), 0, NUM_COINS - 1);
                // Invoke on real caches.
                Coin coin = data.coins[coinidx];
                coin.nHeight = current_height;
            ...
            case coinscache_sim_fuzz::Target_Action::kSpendCoin: { // SpendCoin (moveto = nullptr)
                uint32_t outpointidx = ConsumeIntegralInRange<uint32_t>(currentAction.spendcoin(), 0, NUM_OUTPOINTS - 1);
                // Invoke on real caches.
            ...
            case coinscache_sim_fuzz::Target_Action::kSpendCoinMoveto: { // SpendCoin (with moveto)
                uint32_t outpointidx = ConsumeIntegralInRange<uint32_t>(currentAction.spendcoinmoveto(), 0, NUM_OUTPOINTS - 1);
            ...
            case coinscache_sim_fuzz::Target_Action::kUncache: { // Uncache
                uint32_t outpointidx = ConsumeIntegralInRange<uint32_t>(currentAction.uncache(), 0, NUM_OUTPOINTS - 1);
                // Apply to real caches (there is no equivalent in our simulation).
                caches.back()->Uncache(data.outpoints[outpointidx]);
                break;
            }
            case coinscache_sim_fuzz::Target_Action::kAddCacheLevel: { // Add a cache level (if not already at the max).
            ...
            case coinscache_sim_fuzz::Target_Action::kRemoveCacheLevel: { // Remove a cache level.
                // Apply to real caches (this reduces caches.size(), implicitly doing the same on the simulation data).
                caches.back()->SanityCheck();
                caches.pop_back();
                break;
            }
            case coinscache_sim_fuzz::Target_Action::kFlush: { // Flush.
                // Apply to simulation data.
                flush();
                // Apply to real caches.
                caches.back()->Flush();
            ...
            case coinscache_sim_fuzz::Target_Action::kSync: { // Sync.
            ...
            case coinscache_sim_fuzz::Target_Action::kFlushReallocateCache: { // Flush + ReallocateCache.
                // Apply to simulation data.
                flush();
                // Apply to real caches.
                caches.back()->Flush();
            ...
            case coinscache_sim_fuzz::Target_Action::kGetCacheSize: { // GetCacheSize
                (void)caches.back()->GetCacheSize();
                break;
            }
            case coinscache_sim_fuzz::Target_Action::kDynamicMemoryUsage: { // DynamicMemoryUsage
                (void)caches.back()->DynamicMemoryUsage();
                break;
            }
            case coinscache_sim_fuzz::Target_Action::kChangeheight: { // Change height
                current_height = ConsumeIntegralInRange<uint32_t>(currentAction.changeheight(), 1, current_height - 1);
                break;
            }
        }
    }
