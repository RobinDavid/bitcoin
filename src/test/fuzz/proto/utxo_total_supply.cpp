...
#include <test/fuzz/proto/utxo_total_supply.pb.h>
...
FUZZ_PROTO_TARGET(utxo_total_supply, const utxo_total_supply_fuzz::Target& el) {

    SeedRandomStateForTest(SeedRand::ZEROS);
    const auto mock_time{ConsumeTime(el.time(), /*min=*/1296688602)}; // regtest genesis block timestamp
    ...
    const auto AppendRandomTxo = [&](CMutableTransaction& tx) {
        const auto& txo = txos.at(ConsumeIntegralInRange<unsigned>(el.txoindex(), 0, txos.size() - 1));
    ...
    int64_t duplicate_coinbase_height = ConsumeIntegralInRange<int64_t>(el.duplicatecoinbaseheight(), 0, 300);
    ...
    // and CVE-2018-17144.
    for (int i = 0; i < std::min(el.actions_size(), 2'00); i++) {
        switch(el.actions(i)) {
            default:
            case utxo_total_supply_fuzz::Target::ACTION_APPEND_LAST: {
            ...
            case utxo_total_supply_fuzz::Target::ACTION_ADD_TX: {
            ...
            case utxo_total_supply_fuzz::Target::ACTION_MINE: {
